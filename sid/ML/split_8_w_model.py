...
REF      = "era5"                                   # "none" | "era5" | "mapr"  (targets a/b)
...
def add_reference(df):
    ...
    h_all, t_all = df["height"].values, df["scan_t"].values
    for day, idx in df.groupby("day").groups.items():
        ...
        st, hh = t_all[idx], h_all[idx]
        nearest = np.argmin(np.abs(st[:, None] - rt[None, :]), axis=1)   # closest ref time per row
        within = np.abs(st - rt[nearest]) <= REF_TOL_S
        for k in np.unique(nearest):
            sel = (nearest == k) & within
            if not sel.any():
                continue
            _, rh, rs = prof[k]
            v = np.interp(hh[sel], rh, rs)
            v[(hh[sel] < rh.min()) | (hh[sel] > rh.max())] = np.nan      # only within ERA5 coverage
            ref[idx[sel]] = v
    return ref
...
def plot_correlation(df, cols, title="features"):
    sub = df[cols].replace([np.inf, -np.inf], np.nan).dropna()
    sp = sub.corr(method="spearman")
    print(f"\nSpearman correlation [{title}], n={len(sub)} (|r|>0.8 = redundant):")
...
def main():
    ...
    if REF != "none":
        ...
        if rs is not None:
            df["ref_speed"] = rs
            df["ref_err"] = np.abs(df["vad_speed"] - df["ref_speed"])
    targets = available_targets(df)
    has_model = "ref_err" in df and np.isfinite(df["ref_err"]).any()
    model_cols = features + ["vad_speed", "ref_speed", "ref_err"]
    ...
    if RUN_ALL:
        for w in targets:
            report(get(w))
        plot_correlation(df, features, "VAD quality features")
        if has_model:
            plot_correlation(df, model_cols, f"with {REF.upper()} model")
        for w in targets:
            plot_target(get(w))
        print("\ndone"); return
    ...
    while True:
        ...
        for k, w in enumerate(targets, 1):
            print(f" {k}. {LABEL[w]}")
        print(f" {nt+1}. feature correlation (VAD only)")
        if has_model:
            print(f" {nt+2}. feature correlation (with {REF.upper()} model)")
        print(" 0. quit")
        try:
        ...
        if c == "0":
            break
        if c.isdigit() and 1 <= int(c) <= nt:
            plot_target(get(targets[int(c)-1]))
        elif c == str(nt+1):
            plot_correlation(df, features, "VAD quality features")
        elif has_model and c == str(nt+2):
            plot_correlation(df, model_cols, f"with {REF.upper()} model")
        else:
            print("  not an option")
    print("done")
