...
GOOD_REL  = 0.20         # fraction : |MAPR-VAD|/VAD within this = "good" (in-band)
SPD_FLOOR = 2.0          # m/s : skip the relative band below this (20% of tiny = noise; 0 = off)
# -----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------- plot 4
# "good" = MAPR speed within +-GOOD_REL of VAD speed (relative band around 1:1).
# Evaluated only where VAD speed >= SPD_FLOOR, since the relative band is noise-
# dominated at light winds. Gray = evaluated points, green = in-band,
# black line = in-band fraction per bin.
evalm = S_v >= SPD_FLOOR
rel = np.abs(S_m - S_v) / np.where(S_v > 0, S_v, np.nan)
good = evalm & (rel <= GOOD_REL)
print(f"[in-band] {100*good[evalm].mean():.1f}% within +-{GOOD_REL*100:.0f}% "
      f"of {evalm.sum()} points with VAD speed >= {SPD_FLOOR} m/s")
...
axes.ravel()[0].legend(fontsize=8, loc="upper right")
fig.suptitle(f"MAPR-VAD agreement: speed within +-{GOOD_REL*100:.0f}%  "
             f"({100*good[evalm].mean():.1f}% of {evalm.sum()} pts, VAD>={SPD_FLOOR} m/s)", y=1.0)
plt.tight_layout(); plt.show()
