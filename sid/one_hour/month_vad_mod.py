    ...
    for f in p_files:
        ...
        for k in range(len(era5_agl)):
            ...
            if np.abs(h_valid[j] - era5_agl[k]) > 25:
                continue
            idx = idx_valid[j]
            era5_ws_m.append(era5_ws[k]);  vad_ws_m.append(ws_vad[ti, idx])
            era5_dir_m.append(era5_dir[k]); vad_dir_m.append(wd_vad[ti, idx])
...
if len(era5_ws_m) == 0:
    raise SystemExit("Still zero matches — check surf_alt and era5_agl values.")

# --- stats ---
sdiff = vad_ws_m - era5_ws_m
s_mad, s_sd = np.mean(np.abs(sdiff)), np.std(sdiff)
s_fit = np.polyfit(era5_ws_m, vad_ws_m, 1)
...
d_fit = np.polyfit(era5_dir_m[dmask], vad_dir_m[dmask], 1)
...
ax1.plot(x_ws, np.polyval(s_fit, x_ws), 'r-',  linewidth=1.5, label='fit', zorder=2)
...
ax1.text(0.5, 19,   f"{len(era5_ws_m)} pts, mad: {s_mad:.1f}, sd {s_sd:.1f}")
...
ax2.plot(x_dir, np.polyval(d_fit, x_dir), 'r-',  linewidth=1.5, label='fit', zorder=2)
...
ax2.text(10, 315, f"fit: {d_fit[0]:.1f}x + {d_fit[1]:.0f}")
