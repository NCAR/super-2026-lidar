...
# Partial-dependence API changed across sklearn versions:
#  - >=0.24 / 1.x: PartialDependenceDisplay.from_estimator(...)
#  - older:        plot_partial_dependence(...) module-level function
# On some versions PartialDependenceDisplay exists but lacks from_estimator,
# so check for the method at runtime, not just whether the import succeeds.
def _plot_pd(model, X, feature, ax):
    try:
    ...
    plot_partial_dependence(model, X, [feature], ax=ax)
...
# set to "hrrr" or "mapr" to enable reference-based targets (a_error, b_inband)
REF = "hrrr"
...
LABEL = {"c_speed": "(c) VAD wind speed", "d_passes_strict": "(d) passes strict -33",
         "a_error": "(a) |VAD-ref| rel-error <= thresh", "b_inband": "(b) in-band flag"}
...
def epoch(ds):
    ...
    if "time" in v:
        ...
        if np.nanmax(t) > 1e8:
            return t
        return (float(np.asarray(v["base_time"][...])) + t) if "base_time" in v else t
    return None
...
def load_vad():
    ...
    for dt in sorted(m40):
        ...
        # build per-scan membership in the strict -33 product
        in_strict = None
        if dt in m33:
        ...
        # flatten every scan into per-gate rows (runs for ALL days)
        nt = s.shape[0]
        for i in range(nt):
            ...
            rec = {"day": dt,
                   "height": hc[v],
                   "residual": res[i][v] if res is not None else np.nan,
                   "correlation": cor[i][v] if cor is not None else np.nan,
                   "snr": snr[i][v] if snr is not None else np.nan,
                   "vad_speed": s[i][v],
                   "vad_dir": d[i][v] if d is not None else np.nan,
                   "scan_t": (t40[i] if t40 is not None else np.nan)}
            if w is not None:
                rec["abs_w"] = np.abs(w[i][v]); feats_present.add("abs_w")
            if npts is not None:
                rec["npoints"] = npts[i][v]; feats_present.add("npoints")
            if in_strict is not None:
                rec["passes_strict"] = in_strict[i][v].astype(int)
...
def ref_profiles(date):
    ...
    if REF == "hrrr":
        for f in sorted(glob.glob(os.path.join(HRRR_DIR, date, f"hrrr_profile_{date}_*_ISS1.nc"))):
            ds = nc.Dataset(f)
            ws = np.asarray(ds["wspd"][:], float).ravel()
            wd = np.asarray(ds["wdir"][:], float).ravel()
            ah = np.asarray(ds["height"][:], float).ravel()  # already AGL
            t = int(np.asarray(ds["time"][0]))
            ds.close()
            sp = ws
            ok = np.isfinite(ah) & np.isfinite(sp)
            if ok.sum() > 1:
                o = np.argsort(ah[ok])
                out.append((t, ah[ok][o], sp[ok][o]))
        ...
        if os.path.exists(f):
            ...
            t = epoch(ds)
            hh = to_nan(ds["height"], MAPR_FILL)
            sp = to_nan(ds["wspd"], MAPR_FILL)
            ds.close()
            agl = hh - alt if np.nanmin(hh) > alt - 200 else hh
            for i in range(sp.shape[0]):
                ok = np.isfinite(agl[i]) & np.isfinite(sp[i])
                if ok.sum() > 1:
                    o = np.argsort(agl[i][ok])
                    out.append((t[i], agl[i][ok][o], sp[i][ok][o]))
...
def add_reference(df):
    ...
    for day, grp in df.groupby("day"):
        ...
        for idx in grp.index:
            ...
            if not np.isfinite(tt) or not np.isfinite(hh):
                continue
...
def build_target(df, which):
    ...
    if which in ("a_error", "b_inband"):
        ...
        spd = df["vad_speed"].values
        valid = np.isfinite(ref) & (spd >= SPD_FLOOR)
        rel = np.abs(spd - ref) / np.where(spd == 0, np.nan, spd)
        if which == "a_error":
            # regression on the relative error magnitude
            return rel, "regression", valid & np.isfinite(rel)
        # b_inband: classify whether VAD is within GOOD_REL of the reference
        return (rel <= GOOD_REL).astype(int), "classification", valid & np.isfinite(rel)
    raise ValueError(which)


def available_targets(df):
    t = ["c_speed"]
    if "passes_strict" in df:
        t.append("d_passes_strict")
    if "ref_speed" in df and np.isfinite(df["ref_speed"]).any():
        t += ["a_error", "b_inband"]
    return t
...
def run_target(df, features, which):
    ...
    pi = permutation_importance(mdl, X.iloc[to], y[to], scoring=scoring,
                                n_repeats=10, random_state=0, n_jobs=-1)
    return dict(which=which, task=task, scoring=scoring, features=features,
                X_test=X.iloc[to], y_test=y[to], pred=pred, final=mdl,
                imp_mean=pi.importances_mean, imp_std=pi.importances_std,
                n_train=len(tr), n_test=len(to),
                days_train=len(set(g[tr])), days_test=len(set(g[to])))
...
def plot_target(r):
    ...
    ax[0].set_xlabel(f"perm. importance ({r['scoring']})")
    ...
    if r["task"] == "regression":
        ...
        ax[1].set_xlabel("actual (test)"); ax[1].set_ylabel("predicted")
        ax[1].set_title("Test Fit")
    else:
        fpr, tpr, _ = roc_curve(r["y_test"], r["pred"])
    ...
    _plot_pd(r["final"], r["X_test"], top, ax[2])
...
def plot_correlation(df, features):
    ...
    print("\nSpearman Correlation (|r|>0.8 = redundant):")
