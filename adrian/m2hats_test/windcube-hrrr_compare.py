...
for vad_file in sorted(glob.glob(vad_base + '30min_winds_*.nc')):
  ...
  p_files = sorted(glob.glob(hrrr_base + date + '/hrrr_pressure_' + date + '_*_ISS1.nc'))
  s_files = sorted(glob.glob(hrrr_base + date + '/hrrr_surface_' + date + '_*_ISS1.nc'))
  
  # --- DEBUG ---
  print('vad_file:', vad_file)
  print('date:', date)
  print('p_files found:', len(p_files))
  print('s_files found:', len(s_files))
  ...
  # --- DEBUG: add this block, run once, then remove ---
  sds = nc.Dataset(s_files[0])
  print(sds.variables['z'].units)
  print('surf_alt:', float(sds.variables['z'][0,0,0]) / 9.80665, 'm')
  sds.close()
  break  # stop after the first date so you're not flooded with output
  # --- end debug ---
