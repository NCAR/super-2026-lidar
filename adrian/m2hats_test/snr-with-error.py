...
# Similar to multi-snr-cutoffs.py

# --- Data locations (note: this variant reads from the cnr_33_witherror VAD dataset,
# with filenames prefixed 'VAD_' rather than '30min_winds_') ---
...
# all_ws_pair:  (hrrr_ws, vad_ws) tuple at each matched point, used later for the diagnostic print
#               and to build the direction speed-cutoff mask
all_snr, all_sdiff, all_ddiff_snr, all_ws_pair = [], [], [], []

# Loop over every VAD file (one per day), sorted chronologically
for vad_file in sorted(glob.glob(vad_base + 'VAD_*.nc')):
    ...
    for f in h_files:
        ...
        for k in range(len(hrrr_agl)):
            ...
            all_ws_pair.append((hrrr_ws[k], ws_vad[ti, idx]))      # raw (HRRR, VAD) speed pair, used below in the diagnostic and the direction mask
...
# --- DIAGNOSTIC ---
# Quick sanity check on the overall range of speed differences, and a look at the
# worst offenders to help spot bad matches, unit mismatches, or QC issues
print("sdiff min/max:", all_sdiff.min(), all_sdiff.max())
worst = np.argsort(np.abs(all_sdiff))[-10:]  # indices of the 10 largest |diff| values
print("\n10 largest speed differences (vad_ws, hrrr_ws, diff):")
for i in worst:
    print(f"  vad={all_ws_pair[i][1]:.2f}, hrrr={all_ws_pair[i][0]:.2f}, diff={all_sdiff[i]:.2f}")
# --- END DIAGNOSTIC ---
