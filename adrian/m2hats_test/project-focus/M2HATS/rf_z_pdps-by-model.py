...
def make_pdp_by_model_figure(result, pdp_features, filename, ylabel, grid_res=40):
    """Overlaid partial dependence curves, one per model (HRRR vs ERA5).

    Computed manually (rather than via sklearn's plot_partial_dependence,
    since that function doesn't support conditioning on another feature):
    sweep the target feature across its observed range while pinning the
    'model' flag to 0 (HRRR) or 1 (ERA5) for all rows in the (last-split)
    test set, and average the pooled forest's predictions each time. This
    produces one PDP curve per model so their effects on predicted error
    ...
    fig, axes = plt.subplots(1, len(pdp_features), figsize=(5.5 * len(pdp_features), 4.5))
    if len(pdp_features) == 1:
        axes = [axes]  # normalize to a list so the loop below works for a single feature too

    for ax, feat in zip(axes, pdp_features):
        fi = feature_names.index(feat)
        # sweep between the 5th and 95th percentile of observed values
        lo, hi = np.percentile(Xte[:, fi], [5, 95])
        grid = np.linspace(lo, hi, grid_res)

        for flag, label, color in [(0, 'HRRR', 'steelblue'), (1, 'ERA5', 'indianred')]:
            curve = []
            Xmod = Xte.copy()
            Xmod[:, mflag_idx] = flag          # pin the model flag
            for g in grid:
                Xmod[:, fi] = g                # sweep the feature
                curve.append(rf.predict(Xmod).mean())  # average prediction across all (modified) test rows
            ax.plot(grid, curve, color=color, linewidth=2, label=label)

        # rug marks: deciles of the observed feature values
...
make_pdp_by_model_figure(res_dir, ['lapse', 'hour', 'model_ws'],
                         'rf_z_pdp_bymodel_wind_direction.png',
                         'Predicted |direction error| (deg)')
make_pdp_by_model_figure(res_ws, ['model_ws', 'height'],
                         'rf_z_pdp_bymodel_wind_speed.png',
                         'Predicted |speed error| (m/s)')