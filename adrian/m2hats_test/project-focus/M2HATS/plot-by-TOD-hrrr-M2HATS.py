...
for vad_file in sorted(glob.glob(vad_base + '30min_winds_*.nc')):
    ...
    for f in h_files:
        ...
        for k in range(len(hrrr_agl)):
            ...
            # Note: there's no explicit np.ma.is_masked() check on
            # ws_vad[ti, idx]/wd_vad[ti, idx] here before appending - `valid`
            # above only guarantees ws_vad[ti] has *some* unmasked values,
            # not that this specific idx is unmasked.
...
fig.suptitle('Mean Difference by Hour of Day (VAD - HRRR), M2HATS using VAD consensus', fontsize=16)
