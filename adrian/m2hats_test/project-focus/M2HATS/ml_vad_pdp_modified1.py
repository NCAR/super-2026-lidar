...
def train_and_report(X, y, groups, target_name, n_splits=5):
    ...
    Unlike the single-split versions of this script, this repeats the
    fit/evaluate cycle `n_splits` times with different day-based splits,
    then reports the mean +/- std of R^2 and permutation importance
    across those splits. Only the model/test-set from the LAST split is
    kept (for the PDP figures below) - it isn't a special "best" split,
    just whichever one the loop finished on."""
...
make_table_figure(res_ws,  'rf_table_wind_speed_m2hats_consensus_v3.png')
make_table_figure(res_dir, 'rf_table_wind_direction_m2hats_consensus_v3.png')
...
make_pdp_figure(res_dir, ['lapse', 'hour', 'model_ws'],
                'rf_pdp_wind_direction_m2hats_consensus_v3.png',
                'Predicted |direction error| (deg)')
make_pdp_figure(res_ws, ['model_ws', 'height'],
                'rf_pdp_wind_speed_m2hats_consensus_v3.png',
