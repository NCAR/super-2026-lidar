...
# each row: one matched point. We store predictors + both error targets.
rows = []  # dicts
...
def shear_at(profile_ws, k):
    """Local wind-speed shear (per level) using neighbors, absolute value."""
...
def lapse_at(temp_profile, agl_profile, k):
    """Local temperature lapse (dT/dz) as a stability proxy, K per km.
    Negative (temp decreasing with height) ~ less stable; positive ~ inversion/stable."""
...
def collect(model_name, m_ws, m_dir, m_agl, m_temp, et,
    ...
    for k in range(len(m_agl)):
        ...
        ws_err = float(ws_vad[ti, idx] - m_ws[k])

        # direction error only meaningful when both speeds are reasonable
        if m_ws[k] > 2.0 and ws_vad[ti, idx] > 2.0 and not np.ma.is_masked(wd_vad[ti, idx]):
            dir_err = ((float(wd_vad[ti, idx]) - m_dir[k] + 180) % 360) - 180
        else:
            dir_err = np.nan

        rows.append(dict(
            model = 0 if model_name == 'hrrr' else 1,   # model flag
            hour = hour,
            ...
            day = et // 86400,                           # integer day, for grouped split
            ws_err = ws_err,
            dir_err = dir_err,
        ))
...
def train_and_report(X, y, groups, target_name):
    ...
    # grouped split by DAY so correlated samples don't leak
    ...
    # permutation importance on the held-out set (more reliable)
...
train_and_report(X, y_ws,  groups, "WIND SPEED ERROR (m/s)")
train_and_report(X, y_dir, groups, "WIND DIRECTION ERROR (deg)")