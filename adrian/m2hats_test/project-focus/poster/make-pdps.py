...
def train_and_report(X, y, groups, target_name, n_splits=5):
    ...
    perm_means = []
    impuritys = []
    last_rf, last_Xte, last_yte = None, None, None
...
def make_pdp_figure(result, pdp_features, filename, ylabel):
    """Partial dependence plots for chosen features, one panel each (old sklearn API)."""
...
def make_pdp_by_model_figure(result, pdp_features, filename, ylabel, grid_res=40):
    ...
    Computed manually: sweep the target feature across its observed range
    while pinning the 'model' flag to 0 (HRRR) or 1 (ERA5) for all rows,
    averaging the pooled forest's predictions each time.
    """
    rf = result['model']
    Xte = result['X_test']
    mflag_idx = feature_names.index('model')
    ...
    for ax, feat in zip(axes, pdp_features):
        ...
        for flag, label, color, ls in [(0, 'HRRR', '#009E73', '-'), (1, 'ERA5', '#CC79A7', '--')]:
            curve = []
            Xmod = Xte.copy()
            Xmod[:, mflag_idx] = flag          # pin the model flag
            for g in grid:
                Xmod[:, fi] = g                # sweep the feature
                curve.append(rf.predict(Xmod).mean())
...
make_table_figure(res_ws,  'rf_table_wind_speed_m2hats_consensus_v4.png')
make_table_figure(res_dir, 'rf_table_wind_direction_m2hats_consensus_v4.png')
