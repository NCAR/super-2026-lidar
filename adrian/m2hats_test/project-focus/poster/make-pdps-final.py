...
# display labels (with units) for PDP axes; keys match feature_names.
# Kept separate from feature_names so plot axis text can be more descriptive
# than the short internal names used for indexing/dict keys.
feature_labels = {
    'model':    'model',
    'hour':     'hour (UTC)',
    'height':   'height (m AGL)',
    'model_ws': 'model_ws (m/s)',
    'shear':    'shear (m/s)',
...
def train_and_report(X, y, groups, target_name, n_splits=5):
    """Fit a RandomForestRegressor on the given target across multiple
    group-aware train/test splits (grouped by day, so no day's data leaks
    across a split), to get a more robust estimate of R^2 and feature
    importance than a single split would give.

    This repeats the fit/evaluate cycle `n_splits` times with different
    day-based splits, then reports the mean +/- std of R^2 and permutation
    importance across those splits. Only the model/test-set from the LAST
    split is kept (for the PDP figures below) - it isn't a special "best"
    split, just whichever one the loop finished on."""
    ...
    # Generate n_splits independent day-based train/test splits
    gss = GroupShuffleSplit(n_splits=n_splits, test_size=0.25, random_state=42)

    r2_trs, r2_tes = [], []
    perm_means = []
    impuritys = []
    last_rf, last_Xte, last_yte = None, None, None  # kept from the final split, for PDP figures

    for tr, te in gss.split(Xm, ym, gm):
        # Note: shallower/more-regularized forest than the single-split
        # scripts (min_samples_leaf=30, max_depth=12) since this version
        # cross-validates across multiple splits rather than relying on one
        rf = RandomForestRegressor(n_estimators=300, min_samples_leaf=30,
                                   max_depth=12, n_jobs=-1, random_state=42)
        rf.fit(Xm[tr], ym[tr])
        r2_trs.append(rf.score(Xm[tr], ym[tr]))
        r2_tes.append(rf.score(Xm[te], ym[te]))
        perm = permutation_importance(rf, Xm[te], ym[te], n_repeats=10,
                                      random_state=42, n_jobs=-1)
        perm_means.append(perm.importances_mean)
        impuritys.append(rf.feature_importances_)
        last_rf, last_Xte, last_yte = rf, Xm[te], ym[te]

    r2_trs = np.array(r2_trs); r2_tes = np.array(r2_tes)
    perm_means = np.array(perm_means)      # (n_splits, n_features)
    impuritys = np.array(impuritys)

    print(f"\n{'='*55}\nTARGET: {target_name}   ({mask.sum()} usable rows, {n_splits} splits)\n{'='*55}")
    print(f"R^2 train: {r2_trs.mean():.3f} +/- {r2_trs.std():.3f}")
    print(f"R^2 test:  {r2_tes.mean():.3f} +/- {r2_tes.std():.3f}")

    # Average feature importances across splits; the spread reported here
    # is the variability ACROSS SPLITS (not permutation repeats within one split)
    pm = perm_means.mean(axis=0)
    ps = perm_means.std(axis=0)      # spread ACROSS SPLITS now
    im = impuritys.mean(axis=0)

    print("\nPermutation importance (mean +/- std across splits):")
    for name, m, s in sorted(zip(feature_names, pm, ps), key=lambda x: -x[1]):
        print(f"  {name:>10}: {m:.3f} +/- {s:.3f}")

    return dict(name=target_name, n=int(mask.sum()),
                r2_tr=r2_trs.mean(), r2_te=r2_tes.mean(),
                r2_te_std=r2_tes.std(),
                perm_mean=pm, perm_std=ps, impurity=im,
                model=last_rf, X_test=last_Xte, y_test=last_yte)
...
def make_table_figure(result, filename):
    ...
    permutation importance (descending), with the mean R^2 scores (across
    splits) in the title. Renames 'lapse' to the friendlier 'lapse rate'
    for display, without touching the underlying feature_names list."""
    order = np.argsort(result['perm_mean'])[::-1]  # sort features by permutation importance, descending
    col_labels = ['Feature', 'Permutation\nimportance', 'Impurity\nimportance']
    cell_text = []
    for i in order:
        cell_text.append([
            'lapse rate' if feature_names[i] == 'lapse' else feature_names[i],
...
def make_pdp_figure(result, pdp_features, filename, ylabel):
    ...
    sklearn API). Uses the model and test set from the LAST cross-
    validation split in train_and_report() (not an average across splits).
    Pools both HRRR and ERA5 rows together (contrast with
    make_pdp_by_model_figure, which separates the two)."""
    ...
    # Use the friendlier feature_labels (with units) for axis text
    for ax, f in zip(np.ravel(axes), pdp_features):
        ax.set_xlabel(feature_labels[f], fontsize=13)
...
def make_pdp_by_model_figure(result, pdp_features, filename, ylabel,
                             grid_res=40, layout=None):
    """Overlaid partial dependence curves, one per model (HRRR vs ERA5).
    Wraps to a 2-row grid for 4+ features; pass layout=(nrows, ncols) to
    force a shape (e.g. layout=(3, 1) to stack 3 features vertically for
    a poster figure). Computed manually (rather than via sklearn's
    plot_partial_dependence, since that doesn't support conditioning on
    another feature): sweep the target feature across its observed
    5th-95th percentile range while pinning the 'model' flag to 0 (HRRR)
    or 1 (ERA5) for every row in the test set, averaging the pooled
    forest's predictions at each grid point.
    """
    rf = result['model']
    Xte = result['X_test']
    mflag_idx = feature_names.index('model')  # column index of the 'model' (0=HRRR, 1=ERA5) feature

    n = len(pdp_features)
    if layout is not None:
        # Caller-specified grid shape (e.g. (3, 1) for a vertical poster strip)
        nrows, ncols = layout
    else:
        # Default: single row for up to 3 features, else wrap to 2 rows
        ncols = n if n <= 3 else (n + 1) // 2
        nrows = 1 if n <= 3 else 2

    fig, axes = plt.subplots(nrows, ncols, figsize=(5.5 * ncols, 4.5 * nrows))
    # Flatten axes to a 1D array regardless of whether subplots returned a
    # single Axes, a 1D array, or a 2D array (handles the layout=(1,1) case too)
    axes = np.ravel(np.array([axes]))

    for ax, feat in zip(axes[:n], pdp_features):
        fi = feature_names.index(feat)
        # Sweep grid over the 5th-95th percentile range of this feature in the test set
        lo, hi = np.percentile(Xte[:, fi], [5, 95])
        grid = np.linspace(lo, hi, grid_res)

        # Okabe-Ito colorblind-safe palette: green for HRRR, pink for ERA5
        for flag, label, color, ls in [(0, 'HRRR', '#009E73', '-'),
                                       (1, 'ERA5', '#CC79A7', '-')]:
            curve = []
            Xmod = Xte.copy()
            Xmod[:, mflag_idx] = flag  # pin every row's model flag to this model for the sweep
            for g in grid:
                Xmod[:, fi] = g  # set every row's value of this feature to the current grid point
                curve.append(rf.predict(Xmod).mean())  # average prediction across all (modified) test rows
            ax.plot(grid, curve, color=color, linestyle=ls, linewidth=2.2, label=label)

        # Rug-plot-style tick marks along the bottom showing the feature's decile values
        for q in np.percentile(Xte[:, fi], np.arange(10, 100, 10)):
            ax.axvline(q, ymax=0.04, color='black', linewidth=1)

        ax.set_xlabel(feature_labels[feat], fontsize=13)
        ax.set_ylabel(ylabel, fontsize=13)
        ax.tick_params(labelsize=12)
        ax.grid(alpha=0.3)
        ax.legend(fontsize=12)

    # Hide any unused subplot slots (e.g. an odd number of features on a 2-row grid)
    for ax in axes[n:]:
        ax.axis('off')

    fig.suptitle(result['name'] + ' - by model', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(filename, dpi=300, bbox_inches='tight', pad_inches=0.15)
    plt.close(fig)
    print(f"Saved: {filename}")
...
make_table_figure(res_ws,  'rf_z_table_wind_speed_m2hats_consensus_v4.png')
make_table_figure(res_dir, 'rf_z_table_wind_direction_m2hats_consensus_v4.png')

# pooled PDPs
make_pdp_figure(res_dir, ['lapse', 'hour', 'model_ws'],
                'rf_z_pdp_wind_direction_m2hats_consensus_v4.png',
                'Predicted |direction error| (deg)')
make_pdp_figure(res_ws, ['hour', 'height', 'model_ws', 'lapse'],
                'rf_z_pdp_wind_speed_m2hats_consensus_v4.png',
                'Predicted |speed error| (m/s)')

# by-model PDPs (these are the poster figures)
make_pdp_by_model_figure(res_dir, ['lapse', 'hour', 'model_ws'],
                         'rf_z_pdp_bymodel_wind_direction_m2hats_consensus_v4.png',
                         'Predicted |direction error| (deg)',
                         layout=(3, 1))
make_pdp_by_model_figure(res_ws, ['hour', 'height', 'model_ws', 'lapse'],
                         'rf_z_pdp_bymodel_wind_speed_m2hats_consensus_v4.png',
                         'Predicted |speed error| (m/s)')