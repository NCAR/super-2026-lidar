...
# Read site elevation from the first VAD file (valid at LOTOS); needed to
# convert ERA5 geopotential height (MSL) into AGL height for matching.
...
def train_and_report(X, y, groups, target_name):
    ...
    needed downstream to build the table and PDP figures (including the
    fitted model and held-out test set, since the by-model PDP curves
    below need to re-run predictions with the 'model' feature pinned)."""
...
def make_pdp_by_model_figure(result, pdp_features, filename, ylabel, grid_res=40):
    ...
    sweep the target feature across its observed 5th-95th percentile range
    while pinning the 'model' flag to 0 (HRRR) or 1 (ERA5) for every row
    in the test set, and average the pooled forest's predictions at each
    grid point. This produces one PDP curve per model so their effects on
    predicted error can be compared directly on the same axes.
    """
    rf = result['model']
    Xte = result['X_test']
...
# --- run both targets, produce all figures ---
...
make_table_figure(res_ws,  'rf_z_table_wind_speed_lotos_cnr35.png')
make_table_figure(res_dir, 'rf_z_table_wind_direction_lotos_cnr35.png')

make_pdp_figure(res_ws, ['model_ws', 'height'],
                'rf_z_pdp_wind_speed_lotos_cnr35.png',
                'Predicted |speed error| (m/s)')
make_pdp_figure(res_dir, ['height', 'model_ws', 'lapse'],
                'rf_z_pdp_wind_direction_lotos_cnr35.png',
                'Predicted |direction error| (deg)')

make_pdp_by_model_figure(res_ws, ['model_ws', 'height'],
                         'rf_z_pdp_bymodel_wind_speed_lotos_cnr35.png',
                         'Predicted |speed error| (m/s)')
make_pdp_by_model_figure(res_dir, ['height', 'model_ws', 'lapse'],
                         'rf_z_pdp_bymodel_wind_direction_lotos_cnr35.png',
                         'Predicted |direction error| (deg)')