...
# Read site elevation from the first VAD file; needed to convert ERA5
# geopotential height (MSL) into AGL height for matching.
...
def train_and_report(X, y, groups, target_name):
    ...
    across the split), then print R^2 scores plus permutation-based
    feature importance. Returns a dict with everything needed downstream
    to build the table figure (note: unlike the PDP version of this
    script, the fitted model and test arrays are not kept here)."""
...
def make_table_figure(result, filename):
    ...
    for (row, col), cell in table.get_celld().items():
        ...
        if row == 0:
            ...
            cell.set_height(cell.get_height() * 1.6)  # room for 2-line headers
        elif row % 2 == 0:
...
# --- run both targets and produce figures ---
...
make_table_figure(res_ws,  'rf_importance_wind_speed.png')
make_table_figure(res_dir, 'rf_importance_wind_direction.png')